...
import { getLangFromStorage, t } from "@/app/lib/i18n";
    ...
    <>
      ...
      >
        ...
        >
          ...
          >
            {t("calendarTaskFilterTitle", lang)}
          </div>
        </div>
            ...
            {calendarTaskFilter && calendarTaskFilter.length > 0 ? (
              ...
              >
                {calendarTaskFilter.map((id) => {
                  const tTask = tasksDict.byId[id];
                  if (!tTask) return null;
                  return (
                    <li
                      key={id}
                      style={{
...
function CalendarGrid({
  ...
  return (
    ...
    >
      ...
      >
        <div>{t("weeklySun", lang)}</div>
        <div>{t("weeklyMon", lang)}</div>
        <div>{t("weeklyTue", lang)}</div>
        <div>{t("weeklyWed", lang)}</div>
        <div>{t("weeklyThu", lang)}</div>
        <div>{t("weeklyFri", lang)}</div>
        <div>{t("weeklySat", lang)}</div>
      </div>
...
export default function CalendarPage() {
  ...
  const monthLabel = formatMonthLabel(year, month, lang);
