...
export default function SongListPage() {
  ...
  const handleAddClick = () => {
    if (!currentChildId) {
      alert(t("songsAlertRegisterChildFirst"));
      return;
    }
  ...
  const handleExportCsv = () => {
    ...
    if (sortedSongs.length === 0) {
      alert(t("songsAlertNoSongsForChild"));
      return;
    }
  ...
  const handleFileChange = (e) => {
    ...
    reader.onload = (event) => {
      ...
      if (!headerOk) {
        alert(t("songsAlertHeaderMismatch"));
        return;
      }

      const imported = parseSongsCsv(text);
      if (imported.length === 0) {
        alert(t("songsAlertNoSongsInFile"));
        return;
      }
      ...
      alert(
        `${t("songsAlertImportSuccessPrefix")}${imported.length}${t("songsAlertImportSuccessSuffix")}`
      );
    };
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          ...
          {isChildMode ? (
            ...
            >
              {currentChild?.name || t("songsNoChildRegistered")}
            </div>
          ) : (
            <select
      ...
      <section>
        ...
        >
          {t("songsRegisteredSongsTitle")}（{currentChild?.name || "―"}）
        </h2>
        <p
          style={{
            fontSize: "11px",
        ...
        {sortedSongs.length === 0 ? (
          ...
          >
                        {t("songsEmptyMessageLine1")}
            <br />
            {t("songsEmptyMessageLine2")}
          </p>
        ) : (
            ...
            >
              <thead>
                <tr style={{ backgroundColor: "#faf5ff" }}>
                  ...
                  >
                    {t("songsHeaderComposer")} {sortIcon("composer")}
                  </th>
                  <th
                    onClick={() => toggleSort("title")}
                    style={sortHeaderStyle("title")}
                  >
                    {t("songsHeaderTitle")} {sortIcon("title")}
                  </th>
                  <th
                    onClick={() => toggleSort("date")}
                    style={sortHeaderStyle("date")}
                  >
                    {t("songsHeaderDate")} {sortIcon("date")}
                  </th>
                  <th
                    onClick={() => toggleSort("place")}
                    style={sortHeaderStyle("place")}
                  >
                    {t("songsHeaderPlace")} {sortIcon("place")}
                  </th>
                  <th style={headerCellStyle}>{t("songsHeaderTotal")}</th>
                  <th style={headerCellStyle}>{t("songsHeaderMemo")}</th>
                  <th style={headerCellStyle}>{t("songsHeaderActions")}</th>
                </tr>
              </thead>
              <tbody>
                    ...
                    <tr key={index}>
                      ...
                      >
                        <button
                          ...
                          aria-label={t("songsEditAria")}
                          title={t("songsEditAria")}
                        >
