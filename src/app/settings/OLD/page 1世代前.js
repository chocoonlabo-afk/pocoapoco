...
export default function SettingsPage() {
  ...
  useEffect(() => {
    try {
      ...
      const savedLang = localStorage.getItem("pocopoco_lang");
  ...
  function handleSaveLang() {
    localStorage.setItem("pocopoco_lang", lang);
  ...
  function handleAddTask() {
    ...
    if (!label || !icon) {
      setStatusMsg("アイコンとタスクめいをいれてください");
      return;
    }
    const safeId =
      "task_" +
  ...
  function handleFileSelected(e) {
    ...
    reader.onload = (ev) => {
      try {
        ...
        setRecords(merged);
        setStatusMsg(`CSVから${imported.length}件インポートしました`);
      } catch (err) {
        console.error(err);
        setStatusMsg("CSVの読み込みに失敗しました");
      }
    };
    ...
    >
      ...
      {role === "parent" && (
        ...
        >
          ...
          {children.length === 0 ? (
            <div style={{ fontSize: "12px", color: "#777" }}>登録なし</div>
          ) : (
            children.map((ch) => (
              <div
                key={ch.id}
              ...
              >
                ...
                <div style={{ display: "flex", gap: "4px" }}>
                  <button onClick={() => handleSetCurrentChild(ch.id)} style={btnMini}>
                    この子を表示
                  </button>
                  <button
      ...
      <section style={cardStyle}>
        <div style={sectionTitleStyle}>ひょうじげんご</div>
        <select
          value={lang}
      ...
      {/* ホームに出すイベント */}
      <section style={cardStyle}>
        <div style={sectionTitleStyle}>イベント / 本番の日（ホームに1つ）</div>
        <input
          type="text"
          value={eventTitle}
      ...
      {role === "parent" && (
        <>
          {/* 親コード */}
          <section style={cardStyle}>
            <div style={sectionTitleStyle}>おとなのひみつコード（4けた）</div>
            <input
              type="password"
              value={parentCode}
      ...
      >
        ...
        <p style={{ fontSize: "12px", color: "#666", marginBottom: "8px" }}>
          いちどログイン画面にもどって、もういちど 親 / 子 をえらびなおします。
        </p>
        <button
