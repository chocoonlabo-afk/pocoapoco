...
export default function HistoryEditPage() {
  ...
  // 言語設定を読み込み
  useEffect(() => {
    const initialLang = getLangFromStorage();
    setLang(initialLang);
  }, []);
        ...
        {needAuth ? (
          ...
          <>
            ...
            <form onSubmit={handleSave}>
              ...
              <label style={labelStyle}>
                {t("historyEditTimeLabel", lang)}（
                {t("historyEditMinutesLabel", lang)}）
                <input
                  type="number"
                  min={0}
                  value={minutes}
              ...
              <label style={labelStyle}>
                {t("historyEditStartingTimeLabel", lang)}
                <span
                  style={{
                    display: "block",
                    fontSize: "11px",
                ...
                >
                  {t("historyEditStartingTimeNote", lang)}
                </span>
                <input
                  type="text"
                  value={startedAtStr}
              ...
              >
                ...
                {role === "parent" && (
                  <button
                    type="button"
                    onClick={() => (window.location.href = ROUTE_HISTORY)}
                    style={secondaryButtonStyle}
                  >
                    {t("historyEditBackButton", lang)}
                  </button>
                )}
                <button type="submit" style={primaryButtonStyle}>
                  {t("historyEditSaveButton", lang)}
                </button>
              </div>
            </form>
          </>
